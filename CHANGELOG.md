...
- **A pre-registered real-world benchmark** (`benchmarks/`). Every scoring rule
  was fixed in `benchmarks/PREREGISTRATION.md` before any detector ran. Bots are
  measured at rising levels of evasion (`L0` announces itself, `L5` is a real
  headless browser pacing like a human from rotating IPs) against baselines an
  operator could deploy instead — a User-Agent blocklist, a rate limit, a path
  blocklist, and CrowdSec — plus real production nginx traffic from the public
  Zanbil e-commerce log. Ground truth never comes from the rules under test. The
  report is generated by `python -m benchmarks.report` into
  `docs/results/2026-09-benchmark.md`, and every number traces to a JSON under
  `benchmarks/results/`.
- **A realistic training set, and the shipped model retrained on it**
  (`microguard/training/build_realistic_dataset.py`). The old human class came
  from a single file, so about 8 feature columns were constant across it: the
...
- **A one-command real-human collection session behind an HTTPS tunnel**
  ...
  first stated limitation; this is the path to real ones.

### Fixed

- **The live path now reads the forwarded `Referer`.** The check server and the
  ASGI and WSGI middleware each built every request with `referer=""`; all three
  now read the real header (the check server via
  `proxy_set_header Referer $http_referer;`). Referer-based rules and features
  were inert on the live path, so the "no referrer on all requests" rule fired
  on any 20-plus-request session regardless of what the client actually sent,
  and the human navigation rule could never fire live.
- **The volume rules count page-like requests, not embedded assets.** A browser
  loading one rich page issues dozens of requests for it. Counting them as
  traffic cut the `scan` false-positive rate on real Zanbil human shoppers from
  **57% to 15%**; a page or endpoint scraper is still caught.
- **Credential and API-key brute-force is promoted 0.75 to 0.90**, so the one
  unambiguous-attack rule the 0.85 blend was discarding now blocks on the live
  path. A blanket threshold drop was rejected as unsafe: the `>100 requests`
  rule sits at exactly 0.85 by design.
- **`probe_url` and `probe_websocket` measure elapsed time with
  `time.perf_counter()`, not `time.time()`.** Every one of those values is a
  span, and `time.time()` has a roughly 15.6 ms resolution on Windows, so a
  loopback probe completing inside one tick reported exactly `0.0` — which
  failed `tests/test_scanner_http.py`'s `result.timing['total'] > 0` on
  `windows-latest` / 3.10 while every other platform passed. Same shape as the
  two platform-parity failures already recorded in `CLAUDE.md`: a test that
  depended on the machine rather than the code.
- **The benchmark report no longer contradicts itself about the `Referer`.** Its
  "Two honest limitations up front" section described the forwarded-header
  defect as current while "Fixes applied since this benchmark", in the same
  generated document, said all three entrypoints now read it. The limitation is
  now stated in the past tense as a caveat on the numbers, which are
  deliberately left as they were measured, and links to the fix.

### Changed

- Package coverage is **100%** (`microguard`, all 4,501 statements), up from
  99.31%. CI still gates at 98%.
